...
const ProjectPage = ({ image, hasDemo, title, repo, link, description } : ProjectPageProps) => {
  return (
    <article className='grid place-content-center' data-aos="fade-down" data-aos-once="true" >
      ...
      <div className="sm:w-[310px] w-[650px] rounded-[90px] sm:rounded-[65px] bg-dark2 flex flex-col mt-[150px] pt-[15px] sm:pt-0 pb-[50px] sm:pb-[25px]">
        ...
        <img src={image} alt="project" className="sm:w-[290px] w-[500px] rounded-[50px] mt-[20px] self-center" />
        {hasDemo ? 
          ...
          <div className="self-center my-[20px] text-[1.1rem] font-bold flex">
            <a href={repo} target='_blank' rel='noreferrer' className='active:opacity-100 h-10 border-solid border-light border-2 p-2 items-center rounded-3xl text-light flex no-underline hover:opacity-70 opacity-100 '> <FaGithub />&nbsp; Github Repository </a>
          </div>
        }

        <div className="mx-[40px] sm:mx-[20px] text-[1.2rem]">
          &nbsp;&nbsp;&nbsp;{description}
        </div>
      </div>
    </article>
